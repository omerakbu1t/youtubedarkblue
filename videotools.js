				...
				function cargarScript() {
					console.log('Script enjection by Mapus Limited');
					const HEADER_STYLE =
				...
				console.log(
					...
					#mapus {
					animation: mapus 10s infinite alternate;
					position: relative;
					transition: 4s;
					}
					...
					<button type="button" title="settings colors" style="display:none !important;"class="collapsible botones_div collapsible_colors">
					...
					<button title="Image video" class="botones_div"  style="display:none !important;"type="button" id="imagen">
					...
					const modeReverse = `
					<button style="display:none !important;" title="Reverse" class="botones_div" type="button" id="invertir">
					...
					<div style="display:none !important;" style="position:relative; display:inline-block ">
				...
				<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-circle-x" width="24"
					...
					const checkUpdates = `
					<button  style="display:none !important;"title="Check new updates" type="button" class="checked_updates botones_div">
				...
				<div style="display: flex; margin-top: 5px;">
				<h2>Menu Color:</h2>
				<button title="Text color" class="botones_div" type="button">p
				...
				<select class="selectcalidadesaudio ocultarframeaudio" required>
					...
					let colores = initializecolors();
					return colores[colorId];
					}
				
					function SaveColors(colores) {
					...
					styles_colors = `
				:root {
					--icons-color: ${getColorById('color_icons_color')};
					--progress-bar-video: ${getColorById('color_progress_bar')};
					--header-background-color: ${getColorById('color_header_background')};
					#contents {
					position: sticky;
				...
				.yt-spec-button-shape-next--mono.yt-spec-button-shape-next--tonal {
					...
					applyStyles(
						...
						`${getColorById('color_secondary')}`,
						'--yt-spec-text-secondary'
					);
					// Icons color
					applyStyles(
						propiedadExiste('color_icons_color'),
						`${getColorById('color_icons_color')}`,
					...
					if (addButton != undefined && validoBotones) {
						...
						color_primary.value = getColorById('color_primary')
						? getColorById('color_primary')
						: '#ffffff';
						color_progress_bar.value = getColorById('color_progress_bar')
						? getColorById('color_progress_bar')
						: '#ffffff';
						color_header_background.value = getColorById(
						'color_header_background'
						)
						? getColorById('color_header_background')
						: '#ffffff';
						color_secondary.value = getColorById('color_secondary')
						? getColorById('color_secondary')
						: '#ffffff';
						color_icons_color.value = getColorById('color_icons_color')
						? getColorById('color_icons_color')
						: '#ffffff';
						color_menu.value = getColorById('color_menu')
						? getColorById('color_menu')
						: '#ffffff';
						color_line_preview.value = getColorById('color_line_preview')
						? getColorById('color_line_preview')
						: '#ffffff';
						color_time_preview.value = getColorById('color_time_preview')
						? getColorById('color_time_preview')
						: '#ffffff';
				...
				async function traductor() {
					...
					botonTraducir.classList.add('mapus');
					...
					const mapus = document.querySelectorAll(`.mapus`);
					mapus[i].onclick = function () {
						traducido = o;
						...
						.then((datos) => {
							...
							mapus[i].textContent = 'Translated';
						});
					};
					}
				}
